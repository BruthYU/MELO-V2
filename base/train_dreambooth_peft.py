...
def run(config):
    ...
    if config.args.lora.use_lora:
        lora_config = LoraConfig(
            r=config.args.lora.lora_r,
            lora_alpha=config.args.lora.lora_alpha,
            target_modules=list(config.args.lora.UNET_TARGET_MODULES),
            lora_dropout=config.args.lora.lora_dropout,
            bias=config.args.lora.lora_bias,
        )
    ...
    elif config.args.train_text_encoder and config.args.lora.use_lora:
        lora_config = LoraConfig(
            r=config.args.lora.lora_text_encoder_r,
            lora_alpha=config.args.lora.lora_text_encoder_alpha,
            target_modules=list(config.args.lora.TEXT_ENCODER_TARGET_MODULES),
            lora_dropout=config.args.lora.lora_text_encoder_dropout,
            bias=config.args.lora.lora_text_encoder_bias,
        )
    ...
    if accelerator.is_main_process:
        if config.args.lora.use_lora:
            ...
            # unwarpped_unet = unwrap_peft(unwarpped_unet)
            ...
            if config.args.train_text_encoder:
                ...
                # unwarpped_text_encoder = unwrap_peft(unwarpped_text_encoder)
