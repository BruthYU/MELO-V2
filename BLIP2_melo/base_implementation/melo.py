...
class MeloModel(torch.nn.Module):
    ...
    def set_grace_store_mode_clip(self,mode):
        for module in self.model.modules():
            if isinstance(module, GraceLayer):
                module.store = mode

    def init_dino(self,dino,proccessor):
        for module in self.model.modules():
            if isinstance(module, GraceLayer):
                module.dino = dino
                module.dino_proccessor = proccessor

    def init_flagembedding(self,model):
        for module in self.model.modules():
            if isinstance(module, GraceLayer):
                module.flagembedding = model

    def get_image(self,image,text_input):
        for module in self.model.modules():
            if isinstance(module, GraceLayer):
                module.image = image
...
class VecDB:
    ...
    def update_cluster(self, index, new_key, new_value,id):
        self.table[index]['points'].append(mem_point(new_key,new_value,id))
    ...
    def split_cluster_radii_in_half(self, nearest_cluster, smallest_distance):
        ...
        if len(filtered_key_list) == 0:
            ...
            filtered_key_list.append(mem_point(cluster_center,NO_LORA,-100))
    ...
    def norm(self, dist):
        ##vqa_test
        #return (dist-(0.1047))/(1.0010-0.1047)
        ##caption_test
        #return (dist-(0.0567))/(1.0010-0.0567)

        ##vqa_trian
        return (dist-(0.1047))/(1.0000-0.0919)
        ##caption_trian
        #return (dist-(0.0933))/(1.0000-0.0933)
    

    def euc(self, a: torch.Tensor, b: torch.Tensor):
        ...
        #return logit_scale - torch.mm(a_norm, b_norm.transpose(0, 1))
    
    def cos(self, batch_query, key):
        if isinstance(batch_query, list):
            batch_query = torch.stack(batch_query,dim=0)
        # Euclidean distance
        if len(key.shape) < 2:
            key = key.view(1, -1)
        return torch.nn.functional.cosine_similarity(batch_query, key,dim=1)
    ...
    def search_combine_database(self, batch_query):
        dists = []
        for x in self.table:
            temp= self.euc(batch_query,x['cluster_center'])
            dists.append(temp)
        dists_topk=torch.stack(dists).transpose(0, 1).squeeze(-1)
        dists = torch.stack(dists).view(-1, len(batch_query))

        topk_smallest_distance_list=[]
        topk_nearest_cluster_list = []
        for row in dists_topk:
            k = min(row.numel(), 32)
            values, indices = torch.topk(row, k)
            topk_smallest_distance_list.append(values)
            topk_nearest_cluster_list.append(indices)
        topk_smallest_distance_list = torch.stack(topk_smallest_distance_list)
        topk_nearest_cluster_list = torch.stack(topk_nearest_cluster_list)
        smallest_distance_list, nearest_cluster_list = dists.min(0)
        return smallest_distance_list,nearest_cluster_list, topk_smallest_distance_list, topk_nearest_cluster_list 
    ...
    # def search_vision_text_cluster(self, batch_query, smallest_distance_list,  nearest_cluster_list, image_id):
    #     lora_mapping_block = []
    #     for query, smallest_distance, nearest_cluster, candidate_image in zip(batch_query, smallest_distance_list,nearest_cluster_list,image_id):
    #         try:
    #             #if smallest_distance > self.table[nearest_cluster]['radius']:
    ...
    #             dists = self.euc(key_list,query).view(-1,1)
    #             _, nearest_key = dists.min(0)
    #             if self.table[nearest_cluster]['points'][nearest_key].get_id() in candidate_image:
    #                 lora_mapping_block.append(self.table[nearest_cluster]['points'][nearest_key].get_value())
    #             else:
    #                 lora_mapping_block.append(NO_LORA)
    #         except Exception as e:
    ...
    def search_vision_text_cluster(self, text_dists, vision_dists):
        ...
        for value,id in zip(values,indices_list):
            #if value[0] > 0.4:vqa效果好的那一组只有问题
            #if value[0] > 0.55:caption效果好的那一组
            #if value[0] > 0.28:vqa_ori
            #if value[0] > 0.5:caption_ori
            #if value[0] > 0.4:#用traincaption的阈值   
            if value[0] > 0.35:#用trainvqa的阈值
                lora_mapping_block.append(NO_LORA)
            else:
                lora_mapping_block.append(self.table[id[0]]['points'][0].get_value())
        return lora_mapping_block

    def bind(self, values1, indices_list1, values2, indices_list2):
        bind_list=[]
        result=[]
        for i,iter in enumerate(indices_list1):
            temp1=iter
            temp2=indices_list2[i]
            temp3=list(set(temp1) & set(temp2))
            if temp3:
                temp4=[]
                bind_list.append(temp3)
                for j in temp3:
                    temp4.append(2-(values1[i][temp1.index(j)] + values2[i][temp2.index(j)]))
                result.append(temp4)
            else:
                bind_list.append([])
                result.append([])
        return bind_list, result


    def bind_topk(self, bind_lists, result):
        values1, indices_list1=[],[]
        for sublist,bind_list in zip(result,bind_lists):
            if len(sublist)==0:
                values1.append(100)
                indices_list1.append(100)
            else:
                max_value = min(sublist)
                max_index = sublist.index(max_value)
                values1.append(max_value)
                indices_list1.append(bind_list[max_index])
        return values1, indices_list1
    
    def search_vision_text_cluster_v2(self, text_dist, text_id, vision_dist, vision_id):
        lora_mapping_block = []
        text_dist=text_dist.to('cpu')
        vision_dist=vision_dist.to('cpu')
        bind_list, result = self.bind(text_dist.tolist(), text_id.tolist(), vision_dist.tolist(), vision_id.tolist())
        values1, indices_list1 = self.bind_topk(bind_list, result)
        for r,id in zip(values1, indices_list1):
            if r==-100 or r>0.8:
                lora_mapping_block.append(NO_LORA)
                continue
            else:
                lora_mapping_block.append(self.table[id]['points'][0].get_value())
...
class VisionVecDB:
    ...
    def update_cluster(self, index, new_key, new_value,id):
        ...
        dists = self.euc(key_list, new_cluster_center).view(-1,1)
        largest_distance, _ = dists.max(0)
        self.table[index]['radius'] = max(largest_distance, torch.tensor(self.config['init_vision_radius'], device = new_key.device).view(1))
    ...
    def split_cluster_radii_in_half(self, nearest_cluster, smallest_distance):
        ...
        cluster_center = self.table[nearest_cluster]['cluster_center']
        #dists = self.euc(key_list,cluster_center).view(-1,1)
        dists = self.cos_sim(key_list,cluster_center).view(-1,1)
        filtered_key_list = []
        for index, dist in enumerate(dists):
            if dist <= cluster_radius:
                filtered_key_list.append(self.table[nearest_cluster]['points'][index])
    ...
    def norm(self, dist):
        ##vqa_test
        #return (dist-(-0.4262))/(1.0-(-0.4262))
        ##caption_test
        #return (dist-(0.0367))/(1.0-(0.0367))
    
        ##vqa_train
        #return (dist-(-0.4940))/(1.0-(-0.4940))
        ##caption_train
        #return (dist-(-0.4902))/(1.0-(-0.4902))
    
        ##vqa_train_clip
        #return (dist-(-0.0115))/(1.0-(-0.0115))
        ##vqa_train_vit
        return (dist-(-0.2437))/(1.0-(-0.2437))
    ...
    def search_database(self, batch_query):
        ...
        for row in dists_topk:
            k = min(row.numel(), 100)
            values, indices = torch.topk(row, k, largest=False) 
            topk_smallest_distance_list.append(values)
            topk_nearest_cluster_list.append(indices)
        topk_smallest_distance_list = torch.stack(topk_smallest_distance_list)
        topk_nearest_cluster_list = torch.stack(topk_nearest_cluster_list)
    ...
    def search_vision_cluster(self, batch_query, smallest_distance_list,  nearest_cluster_list):
        image_id = []
        nearest_cluster_list=nearest_cluster_list.tolist()
        for nearest_cluster in nearest_cluster_list:
            temp=[]
            for iter in nearest_cluster:
                id_list = [x.get_id() for x in self.table[iter]['points']]
                temp=temp+id_list
            image_id.append(temp)
        return image_id
    

    def search_vision_cluster_v2(self, batch_query, smallest_distance_list,  nearest_cluster_list):
        image_id = []
        dists = []
        nearest_cluster_list=nearest_cluster_list.tolist()
        for nearest_cluster in nearest_cluster_list:
            ...
            for iter in nearest_cluster:
                ...
                key_list = [x.get_key() for x in self.table[nearest_cluster]['points']]
                key_list = torch.stack(key_list, dim=0)
                dist = self.cos_sim(batch_query,key_list)
                temp=temp+id_list
            image_id.append(temp)
        return image_id
...
class GraceLayer:
    def __init__(self, grace_config: dict, in_features: int, out_features: int, **kwargs):
        ...
        self.VisionVecDB = VisionVecDB(grace_config)
        ...
        self.store = None
        self.dino = None
        self.dino_proccessor = None
        self.flagembedding = None
        self.image = None
        self.text_input = None
        self.image_id = None
        from transformers import AutoModelForCausalLM, AutoModelForSeq2SeqLM
        language_model = AutoModelForCausalLM.from_pretrained('/home/hy/Yjh/EasyEdit-main/base_model/opt_model')
    ...
    def search(self, batch_query,batch_query_vison):
        if batch_query_vison is None:
            ...
            text_dists = self.VecDB.search_combine_databasev2(batch_query)
            vision_dists = self.VisionVecDB.search_combine_databasev2(batch_query_vison)
        lora_block_mapping = self.VecDB.search_vision_text_cluster(text_dists.to('cpu'),vision_dists.to('cpu'))
        return lora_block_mapping

    # def search(self, batch_query,batch_query_vison):
    #     smallest_distance_list, nearest_cluster_list,topk_smallest_distance_list_text, topk_nearest_cluster_list_text = self.VecDB.search_combine_database(batch_query)
    #     smallest_distance_list_vison, nearest_cluster_list_vision, topk_smallest_distance_list_viosn, topk_nearest_cluster_list_vision = self.VisionVecDB.search_combine_database(batch_query_vison)
    #     lora_block_mapping = self.VecDB.search_vision_text_cluster_v2(self.VecDB.norm(topk_smallest_distance_list_text), topk_nearest_cluster_list_text,self.VisionVecDB.norm(topk_smallest_distance_list_viosn), topk_nearest_cluster_list_vision)
    #     return lora_block_mapping
    ...
    def init_key_value(self, batch_query, vision_batch_query):
        lora_block_mapping = []
        for index, query in enumerate(batch_query):
            ...
            self.VecDB.add_cluster(new_key = new_key, new_value = new_value, new_edit_label = new_eidt_label, id = new_id)
            #vision
            new_key = vision_batch_query[index].detach()
            new_eidt_label = self.image_id[index]
            new_value = self.current_block()
            new_id = self.current_id()
            self.VisionVecDB.add_cluster(new_key = new_key, new_value = new_value, new_edit_label = new_eidt_label, id = new_id)
    ...
    def add_cluster(self, query, label_index,id):
        new_key = query.detach()
        new_value = self.current_block()
        new_edit_label = self.edit_label[label_index]
        self.VecDB.add_cluster(new_key = new_key, new_value = new_value, new_edit_label=new_edit_label,id=id)

    def add_vision_cluster(self, query, label_index,id):
        new_key = query.detach()
        new_value = self.current_block()
        new_edit_label = self.image_id[label_index]
        self.VisionVecDB.add_cluster(new_key = new_key, new_value = new_value, new_edit_label=new_edit_label,id=id)

    
    def update_cluster(self, index, query, value, id):
         self.VecDB.update_cluster(index,query.detach(), value, id)

    def update_vision_cluster(self, index, query, value, id):
         self.VisionVecDB.update_cluster(index, query.detach(), value, id)

    # def proccess_batch_query(self, batch_query):
    #     if batch_query.shape[1]>=50:
    #         batch_query=batch_query[:,:50,:]
    #     else:
    #         pad=torch.ones(batch_query.shape[0],50-batch_query.shape[1], dtype=torch.int).to(batch_query.device)
    #         pad_embeds = self.language_model.get_input_embeddings()(pad)
    #         batch_query = torch.cat([batch_query, pad_embeds], dim=1)
    #     return batch_query
...
###原始版本
    ...
    #     batch_query = [torch.from_numpy(self.flagembedding.encode(x)).unsqueeze(0).float() for x in self.text_input]
    #     batch_query  = torch.cat(batch_query,dim=0)
    #     if self.image is not None:
    #         image_preprocess = [self.dino_proccessor(x).unsqueeze(0).to("cuda") for x in self.image]
    #         image_preprocess  = torch.cat(image_preprocess,dim=0)
    #         batch_query_vison= self.dino(image_preprocess)
    #     else:
    #         batch_query_vison = None
    #         self.lora_block_mapping = LORA_BLOCK_MAPPING = [NO_LORA] * layer_out.shape[0]
    #         return layer_out

        
    #     #smallest_distance_list, nearest_cluster_list, lora_block_mapping = None, None, [NO_LORA] * layer_out.shape[0]
    #     smallest_distance_list, nearest_cluster_list, smallest_distance_list_vison, nearest_cluster_list_vision, lora_block_mapping = None, None, None, None, [NO_LORA] * layer_out.shape[0]

    #     if len(self.VecDB) != 0:
    #         smallest_distance_list, nearest_cluster_list, smallest_distance_list_vison, nearest_cluster_list_vision, lora_block_mapping = self.search(batch_query,batch_query_vison)
    ...
    #     '''
    #     if len(self.VecDB) == 0:
    #         self.lora_block_mapping = self.init_key_value(batch_query,batch_query_vison)
    ...
    #                 self.add_cluster(query,label_index=index,id=self.id)
    ...
    #                 self.update_cluster(nearest_cluster_list[index],query,self.block_id,self.id)
    #             else:
    ...
    #             #vision
    #             row = self.VisionVecDB[nearest_cluster_list_vision[index]]
    #             query = batch_query_vison[index]
    #             if smallest_distance_list_vison[index] > row['radius'] + self.init_vision_radius:
    #                 self.add_vision_cluster(query,label_index=index,id=self.id)
    #             elif self.VisionVecDB.label_match(self.image_id[index], row['key_label']):
    #                 print(f'The {index}th query is close to a previous edit, the labels are the same')
    #                 self.update_vision_cluster(nearest_cluster_list_vision[index],query,self.block_id,self.id)
    #             else:
    #                 print(f'The {index}th query is close to a previous edit, but the labels are different')
    #                 self.add_vision_cluster(query,label_index = index,id=self.id)
    #                 self.VisionVecDB.split_cluster_radii_in_half(nearest_cluster_list_vision[index], smallest_distance_list_vison[index])
    #             self.id += 1
    #         self.block_id += 1
    #     else:
    #         pass
...
###删除图片的cluster操作    
    def forward(self, x: torch.Tensor):
        ...
        # batch_query  = torch.cat(batch_query,dim=0)
        # if self.image is not None:
        #     image_preprocess = [self.dino_proccessor(images = x, return_tensors="pt", padding=True)["pixel_values"] for x in self.image]
        #     image_preprocess  = torch.cat(image_preprocess,dim=0)
        #     batch_query_vison= F.normalize(self.dino.get_image_features(image_preprocess),p=2,dim=1)
        # else:
        #     batch_query_vison = None
        ...
        if self.image is not None:
            image_preprocess = [self.dino_proccessor(images = x, return_tensors="pt", padding=True)["pixel_values"] for x in self.image]
            image_preprocess  = torch.cat(image_preprocess,dim=0)
            batch_query_vison= F.normalize(self.dino(pixel_values=image_preprocess).pooler_output,p=2,dim=1)
        else:
            batch_query_vison = None

        
        #smallest_distance_list, nearest_cluster_list, lora_block_mapping = None, None, [NO_LORA] * layer_out.shape[0]
        lora_block_mapping = [NO_LORA] * layer_out.shape[0]

        if len(self.VecDB) != 0:
            lora_block_mapping = self.search(batch_query,batch_query_vison)
